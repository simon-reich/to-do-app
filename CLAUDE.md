...
3. In `TodoCard.vue`s `ALL_CELEBRATIONS`-Map (`Record<CelebrationKey, CelebrationConfig>`) einen Eintrag ergänzen: `{ importer: () => import('../assets/animations/<name>.svg?raw'), size, fallbackCycleMs, verticalAnchor? }` — `size` ist entweder `{ width: <vw> }` oder `{ height: <vh> }`, die jeweils andere Achse folgt per `auto` dem Seitenverhältnis der SVG (Cat: `{ width: 160 }`, bewusst > 100, damit sie links/rechts überläuft statt reinzupassen; Whale: `{ height: 100 }`, damit sie immer die volle Viewport-Höhe annimmt; Penguin: `{ width: 100 }`, volle Breite). `fallbackCycleMs` ist die volle Zyklusdauer aus der `animation:`-Zeile im SVG selbst (z.B. `1.2s` → 1200). `verticalAnchor` ist optional, `'center'` (Default) oder `'bottom'` — Penguin nutzt `'bottom'`, damit die Figur immer am unteren Viewport-Rand steht statt vertikal zentriert zu schweben.
...
**Welche Celebration ein Todo bekommt** wird nicht beim Abspielen zufällig gewählt, sondern einmal beim Senden nach Focus (`sendToToday` in `stores/todos.ts`) fest zugelost und in `Todo.celebration` gespeichert (`CelebrationKey`, Shuffle-Bag `drawCelebrationKey()` in `src/composables/useCelebrations.ts`, aktueller Pool: `cat`, `whale`, `penguin`). Bleibt fix, solange das Todo in Focus ist; verlässt es Focus und kommt später wieder rein, wird neu gelost. `TodoCard.vue`s `ALL_CELEBRATIONS`-Map kennt nur noch, *wie* ein Key gerendert wird (SVG/Größe/Anchor), nicht mehr die Zufallslogik selbst — die lebt bewusst im Store, damit sie keine Kopplung an SVG-/Animations-Code braucht.
